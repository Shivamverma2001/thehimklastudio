...
export async function POST(request: NextRequest) {
  try {
    ...
    const emailTo = process.env.EMAIL_TO || siteConfig.contact.email;
    const smtpHost = process.env.SMTP_HOST;
    const smtpUser = (process.env.SMTP_USER ?? "").trim();
    // Gmail App Passwords are shown as "xxxx xxxx xxxx xxxx" — strip spaces so .env copy-paste works
      ...
      try {
        ...
        await transporter.sendMail({
          ...
          html: [
            ...
            `<p>Best regards,<br/>The Him कLA Studio<br/>${siteConfig.contact.city}</p>`,
          ].join(""),
        });
      } catch (emailError) {
        // Registration and DB save succeeded; only email failed (e.g. bad SMTP credentials)
        console.error("Register API: email send failed (registration was saved):", emailError);
      }
    }
